import { autoCompleteUrl, autoCompleteOptions, autoSuggestionsUrlOptions, autoSuggestionsUrl } from "./utils.js";
...
async function getAutoCompleteSuggestions(inputValue) {
  ...
  if (inputValue === "") {
    ...
    autoCompleteOptions.body=JSON.stringify({text:inputValue,language: 'en'});
    const [suggestionsData,completionsData]= await Promise.all([fetch(
          autoSuggestionsUrl + inputValue,
    ...
    generateSuggestions(suggestions?.data, suggestionsContainer,JSON.parse(completions).predictions);
  }
}
